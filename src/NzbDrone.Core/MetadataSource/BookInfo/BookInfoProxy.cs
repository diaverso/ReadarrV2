...
namespace NzbDrone.Core.MetadataSource.BookInfo
{
    public class BookInfoProxy : IProvideAuthorInfo, IProvideBookInfo, ISearchForNewBook, ISearchForNewAuthor, ISearchForNewEntity
    {
        ...
        private readonly OpenLibraryProxy _openLibraryProxy;
        ...
        public HashSet<string> GetChangedAuthors(DateTime startTime)
        {
            if (UseOpenLibrary)
            {
                return _openLibraryProxy.GetChangedAuthors(startTime);
            }
        ...
        public Tuple<string, Book, List<AuthorMetadata>> GetBookInfo(string foreignBookId)
        {
            if (UseOpenLibrary)
            {
                return _openLibraryProxy.GetBookInfo(foreignBookId);
            }
        ...
        public List<object> SearchForNewEntity(string title)
        {
            if (UseOpenLibrary)
            {
                return _openLibraryProxy.SearchForNewEntity(title);
            }
        ...
        public List<Author> SearchForNewAuthor(string title)
        {
            if (UseOpenLibrary)
            {
                return _openLibraryProxy.SearchForNewAuthor(title);
            }
            ...
            try
            {
                ...
                if (split.Length == 2 && new[] { "author", "work", "edition", "isbn", "asin" }.Contains(prefix))
                {
                    ...
                    if (prefix == "author" || prefix == "work" || prefix == "edition")
                    {
                        ...
                        if (prefix == "edition")
                        {
                            return SearchByForeignEditionId(searchId.ToString(), getAllEditions);
                        }
                    }
        ...
        public List<Book> SearchByIsbn(string isbn)
        {
            if (UseOpenLibrary)
            {
                return _openLibraryProxy.SearchByIsbn(isbn);
            }
        ...
        public List<Book> SearchByAsin(string asin)
        {
            if (UseOpenLibrary)
            {
                return _openLibraryProxy.SearchByAsin(asin);
            }
        ...
        private List<Book> Search(string query, bool getAllEditions)
        {
            ...
            else
            {
                ...
                if (ids.Count == 1)
                {
                    return SearchByForeignEditionId(ids[0].ToString(), false);
                }
        ...
        public List<Book> SearchByForeignEditionId(string foreignEditionId, bool getAllEditions)
        {
            if (UseOpenLibrary)
            {
                return _openLibraryProxy.SearchByForeignEditionId(foreignEditionId, getAllEditions);
            }

            if (!int.TryParse(foreignEditionId, out var id))
            {
                _logger.Warn("Cannot parse edition ID '{0}' as integer for BookInfo lookup", foreignEditionId);
                return new List<Book>();
            }
