...
namespace NzbDrone.Core.MetadataSource.Goodreads
{
    ...
    public class GoodreadsProxy : IGoodreadsProxy, IProvideSeriesInfo, IProvideListInfo
    {
        ...
        private IHttpRequestBuilderFactory BuildRequestFactory(string token)
        {
        ...
        {
            var token = _configService.GoodreadsToken;
            if (token.IsNullOrWhiteSpace())
            {
                _logger.Debug("Goodreads token not configured, skipping series info for {0}", foreignSeriesId);
                return null;
            }

            _logger.Debug("Getting Series with GoodreadsId of {0}", foreignSeriesId);

            var httpRequest = BuildRequestFactory(token).Create()
        ...
        public ListResource GetListInfo(int foreignListId, int page, bool useCache = true)
        {
            ...
            if (token.IsNullOrWhiteSpace())
            {
                _logger.Debug("Goodreads token not configured, skipping list info for {0}", foreignListId);
                return null;
            }
        ...
        public Book GetBookInfo(string foreignEditionId, bool useCache = true)
        {
            ...
            if (token.IsNullOrWhiteSpace())
            {
                _logger.Debug("Goodreads token not configured, skipping book info for {0}", foreignEditionId);
                return null;
            }
