...
namespace NzbDrone.Core.MediaFiles.BookImport.Identification
{
    ...
    public class CandidateService : ICandidateService
    {
        ...
        public IEnumerable<CandidateEdition> GetRemoteCandidates(LocalEdition localEdition, IdentificationOverrides idOverrides)
        {
            ...
            {
                {
                    _logger.Trace($"Searching by edition id {goodreads[0]}");

                    try
                    {
                        remoteBooks = _bookSearchService.SearchByForeignEditionId(goodreads[0], true);
                    }
                    catch (GoodreadsException e)
                    {
                        _logger.Info(e, "Skipping edition ID search due to metadata error");
                        remoteBooks = new List<Book>();
                    }
